...
interface AddLayerClientProps {
  maps: IMaps[] | [];
}

export default function AddLayerClient({ maps }: AddLayerClientProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const resp = await axios.post(`/api/maps/layers/create`, payload, {
        headers: { "Content-Type": "application/json" },
      });
      if (resp.status >= 200 && resp.status < 300) {
        notifications.show({ color: "green", title: "Успешно", message: "Слой создан" });
        router.push(`/dashboard/layers`);
        router.refresh();
      } else {
        const detail = resp?.data?.detail || "Не удалось создать слой";
        throw new Error(detail);
      }
    } catch (e: any) {
      const message = e?.response?.data?.detail || e?.message || "Ошибка создания слоя";
      notifications.show({ color: "red", title: "Ошибка", message });
    }
  };
    ...
    <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="lg:col-span-2">
        <Group justify="space-between" align="center">
          <div>
            <Title order={3}>Добавить слой</Title>
          </div>
        </Group>
        <Divider my="sm" />
      </div>
